    ...
    func share(_ params: [String: Any]?){
        ...
        DispatchQueue.global().async {
            ...
            guard let image_url = URL(string: icon) else { return
                XYGamesKit.sdkShare(mode: mode, title: title, text: text, url: url)
            }
            ...
            DispatchQueue.main.async {
                ...
                XYGamesKit.sdkShare(mode: mode, title: title, text: text, url: url, previewImage: image)
            }
        }
    }
        ...
        NGNet.apiCheckOrder { flag, response, msg, dict in
            if flag {
                Totas.show("购买成功", .success) {
                    XYGamesKit.shared.sdkPayResult(true)
                }
            } else {
