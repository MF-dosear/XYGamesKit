...
@objc public class XYGamesKit: NSObject {
    
    
    typealias NGSDKResult = (_ flag: Bool) -> Void
    
    static let shared = XYGamesKit()
    ...
    @objc public class func sdkInit(app_id: String, app_key: String, link: String, appld_id: String, jy_app_key: String, result: @escaping (_ flag: Bool) -> Void) {
        ...
        /// 初始化结果
        XYGamesKit.shared.sdkInitResult = { flag in
            ...
            if flag {
                ...
                // 分享初始化
                XYGamesKit.shared.sdkShareInit()
        ...
        /// 添加通知
        XYGamesKit.shared.addNotification()
        ...
        /// 极验初始化
        XYGamesKit.shared.initJYSDK(jy_app_key)
        ...
        if #available(iOS 14, *) {
            ...
            ATTrackingManager.requestTrackingAuthorization { status in
                ...
                DispatchQueue.main.async {
                    ...
                    XYGamesKit.shared.sdkCheckVersion(idfa)
                }
            }
    ...
    @objc public class func sdkLogin(automatic:Bool, result: @escaping (_ flag: Bool, _ uid: String?, _ sid: String?, _ user_name: String?) -> Void) {
        ...
        /// 登录结果
        XYGamesKit.shared.sdkLoginResult = { flag in
            ...
            if flag {
                ...
                if NGNet.shared.user.buoyState == "open" {
                    XYGamesKit.shared.tipView = NGTipView()
                    XYGamesKit.shared.tipView?.show()
                }
            ...
            NGNet.apiLogin(username, password) {flag, response, msg ,dict in
                if flag {
                    ...
                    Totas.show("登录成功", .success) {
                        XYGamesKit.shared.checkBindPhone()
                    }
                    
                } else {
                    Totas.show(msg, .fail) {
                        XYGamesKit.shared.sdkLoginResult(false)
                        showLoginView()
                    }
                }
            }
    ...
    @objc public class func sdkPay(role: Role, order: Order, result: @escaping (_ flag: Bool) -> Void) {
        ...
        XYGamesKit.shared.sdkPayResult = { flag in
        ...
        NGNet.apiGetOrderID(&params) { flag, response, msg, dict in
            ...
            if flag {
                ...
                Totas.show(text, .fail) {
                    XYGamesKit.shared.sdkPayResult(false)
                }
            }
        }
    }
    ...
    @objc public class func sdkLogout() {
        ...
        // 悬浮框
        XYGamesKit.shared.tipView?.removeFromSuperview()
        XYGamesKit.shared.tipView = nil
         
        XYGamesKit.shared.sdkLogoutResult(true)
    }
    
    /// 登出回调
    @objc public class func sdkLogoutBack(result: @escaping (_ flag: Bool) -> Void) {
        
        XYGamesKit.shared.sdkLogoutResult = { flag in
...
//                NGNet.shared.config.isShare = data?["isShare"] as? String
    ...
    func checkAuth(){
        ...
        if (is_open_smrz != "1" || is_smrz  == 1) {
            // 没有开启实名认证 或者 实名认证成功
            XYGamesKit.shared.sdkLoginResult(true)
        } else {
            // 实名认证
